...
function get_master_info() {
  ...
  if [ "$(redis-cli -h $REDIS_MASTER_IP -p $REDIS_PORT --user $SENTINEL_USER --pass $SENTINEL_PASSWORD ping)" = "PONG" ]; then
    ...
    local SENTINEL_IPS=$(drill tasks.$SENTINEL_IP | grep tasks.$SENTINEL_IP | tail -n +2 | awk '{print $5}')
    for sentinel_ip in $SENTINEL_IPS; do
      echo "try to get master info from $sentinel_ip"
  ...
  if [ ! $MASTER_INFO ]; then
    until [ "$(redis-cli -h $REDIS_MASTER_IP -p $REDIS_PORT --user $SENTINEL_USER --pass $SENTINEL_PASSWORD ping)" = "PONG" ]; do
      echo "$REDIS_MASTER_IP on port $REDIS_PORT is unavailable - trying again in 5 seconds"
      sleep 5
    done
  fi
}
